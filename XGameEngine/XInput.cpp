...
namespace Xge
{
	...
	{
	  // Clear parent Input object pointer
	  m_Input = NULL;
	...
	IDirectInputDevice8 *InputDevice::DeviceCOM()
	{
	...
	BOOL InputDevice::SetButtonState(char Num, BOOL State)
	{
	  if(m_Type == MOUSE) {
	...
	BOOL InputDevice::GetPureButtonState(char Num)
	{
	  if(m_Type == MOUSE)
