                           ...
                           float MaxSize, long MaxPolygons)
{
  ...
  m_NumGroups   = m_Mesh->GetNumMaterials();
  ...
  D3DXVECTOR3 LeftDown, RightUp;
  float Radius;
  Mesh->GetBounds(&LeftDown, &RightUp, &Radius);
  MaxX = (float)max(fabs(LeftDown.x), 
                    fabs(RightUp.x));
  MaxY = (float)max(fabs(LeftDown.y), 
                    fabs(RightUp.y));
  MaxZ = (float)max(fabs(LeftDown.z), 
                    fabs(RightUp.z));
...
BOOL cNodeTreeMesh::Render(cFrustum *Frustum, float ZDistance)
{
  ...
  m_Mesh->GetMesh()->GetVertexBuffer(&pVB);
  ...
  for(i=0;i<m_NumGroups;i++) {
    ...
    if(m_Groups[i].NumPolygonsToDraw) {
		m_Graphics->GetDeviceCOM()->SetMaterial(&m_Mesh->GetMeshMaterials()[i]);
		m_Graphics->GetDeviceCOM()->SetTexture(0, m_Mesh->GetMeshTextures()[i]);
      ...
      m_Graphics->GetDeviceCOM()->DrawIndexedPrimitive(
								...
								m_Mesh->GetMesh()->GetNumVertices(),0,
  ...
  if(Node->NumPolygons != 0) {
    ...
    for(i=0;i<Node->NumPolygons;i++) {
      ...
      if(Polygon->Timer != m_Timer) {
		...
		if(Group < m_NumGroups && m_Mesh->GetMeshMaterials()[Group].Diffuse.a != 0.0f) {
